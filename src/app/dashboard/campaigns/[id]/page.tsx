...
import { Send, Check, Edit3, ChevronDown, ChevronUp, CheckCircle, Upload, Zap } from 'lucide-react'
...
export default function CampaignDetailPage() {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {campaign.total_contacts} contacts &middot; {emails.length} emails generated &middot; Mode: {campaign.mode || 'simple'}
          </p>
        </div>
      ...
      {/* Step 1: Upload CSV — show when no contacts */}
      {!hasContacts && (
      ...
      {/* Step 2: Generate — show when contacts exist but no emails */}
      {hasContacts && !hasEmails && (
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
          <h2 className="text-lg font-semibold mb-1">Step 2: Generate Personalized Emails</h2>
          <p className="text-sm text-slate-500 mb-4">
            AI will research each of the {campaign.total_contacts} contacts and write unique emails.
          ...
          <button onClick={generateEmails} disabled={generating}
            className="flex items-center gap-2 bg-orange-500 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-orange-600 transition disabled:opacity-50">
            {generating ? (
              <><Zap className="w-4 h-4 animate-pulse" /> Generating... (this takes a few minutes)</>
            ) : (
              <><Zap className="w-4 h-4" /> Generate {campaign.total_contacts} Emails</>
            )}
          </button>
        </div>
      )}

      {/* Stats bar — show when emails exist */}
      {hasEmails && (
        <>
